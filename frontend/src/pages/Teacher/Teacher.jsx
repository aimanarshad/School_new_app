...
function Teacher() {
  ...
  // Direct delete from table row (with browser confirmation)
  const handleDeleteFromTable = async (teacher) => {
    if (!window.confirm(`Delete ${teacher.name} (${teacher.subject || "unknown subject"}) ?`)) {
      return;
    }
  ...
  // Modal-based delete (used when clicking the Delete Teacher card)
  const handleDeleteFromModal = async () => {
    if (!deleteId) {
      toast.warn("No teacher selected");
      return;
    }

    const teacher = teachers.find((t) => t._id === deleteId);
    if (!teacher) {
      toast.error("Teacher not found");
      return;
    }

    if (!window.confirm(`Delete ${teacher.name} (${teacher.subject || "unknown subject"}) ?`)) return;
    ...
    <div>
      ...
      >
        <form id="addTeacherForm" onSubmit={handleAddSubmit}>
          ...
          <div className="mb-3">
            <label className="form-label">Salary *</label>
            <input name="salary" value={formData.salary} onChange={handleChange} className="form-control" required type="number" min="0" step="1" />
          </div>
        </form>
      </Modal>
      ...
      {/* Delete Modal (used only when clicking the "Delete Teacher" card) */}
      <Modal
        show={activeModal === "delete"}
        onClose={() => setActiveModal(null)}
        title="Delete Teacher"
      ...
      >
        <div className="mb-3">
          ...
          <input
            ...
            placeholder="Enter teacher _id"
            disabled
          />
          <small className="text-muted mt-2 d-block">
            (Select a teacher from the table to delete)
          </small>
        </div>
      </Modal>

      <h2 className="main-title mt-5">Teachers List</h2>

      {loading ? (
        <p className="text-center py-5">Loading teachers...</p>
      ) : (
        <TeacherDataTable
          data={teachers}
          onEdit={openUpdateModal}
